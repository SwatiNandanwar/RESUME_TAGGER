...
function adder(event, element) {
  if (event.key === 'Enter') {
    if (element == 'skill') {
      ...
      $('#add_Cert').click();
      document.getElementsByClassName('Cert')[$('.Cert').length - 1].focus();
    }
  }
}
...
function updateProj() {
  for (let i = 0; i < $('#accordionProj .accordion-item').length; i++) {
    let a = ($(`#accordionProj .accordion-item:nth-child(${i + 1}) .Proj_title`).val().trim() == '') ? 'Project' : $(`#accordionProj .accordion-item:nth-child(${i + 1}) .Proj_title`).val().trim();

    let c = ($(`#accordionProj .accordion-item:nth-child(${i + 1}) .Tech`).val().trim() == '') ? '' : ' at ' + $(`#accordionProj .accordion-item:nth-child(${i + 1}) .Tech`).val().trim();

    $(`#accordionProj .accordion-item:nth-child(${i + 1}) .accordion-button`).html(a + c);
  }
}

function pmakeVisible() {
  $("#accordionProj .accordion-header").css("display", "block");
  updateProj();
}

function delProj2(event) {
  event.preventDefault();
  if ($("#accordionProj .accordion-item").length > 1) {
    pmakeVisible();
    event.target.parentElement.parentElement.parentElement.remove();
  }
  event.stopPropagation();
...
let ProjectAdder = $("#accordionProj").html();
let ProjectCounter = 1;

$("#add_Proj").click(function (e) {
  let isValid = true;
  let finalValid = true;
  $("#accordionProj .accordion-item:last-child").find("input").each(function () { isValid = validate_chg_color(this); if (!isValid) { finalValid = false; } });
  ...
  if (!finalValid) {
    ...
    updateProj();
    ProjectCounter++;
    if ($("#accordionProj .accordion-item").length > 0) {
      $("#accordionProj .accordion-header").css("display", "block");
      let count = $("#accordionProj .accordion-item").length;
      if (document.getElementById("accordionProj").getElementsByClassName("accordion-item")[count - 1].getElementsByClassName("accordion-collapse")[0].classList.contains("show")) {
        document.getElementById("accordionProj").getElementsByClassName("accordion-item")[count - 1].getElementsByClassName("accordion-button")[0].click();
      }
    }
    $("#accordionProj").append(ProjectAdder);
    $("#accordionProj .accordion-header").last().attr("id", "pheading" + ProjectCounter);
    $("#accordionProj .accordion-collapse").last().attr("aria-labelledby", "pheading" + ProjectCounter);
    $("#accordionProj .accordion-collapse").last().attr("id", "pcollapse" + ProjectCounter);
    $("#accordionProj .accordion-button").last().attr("data-bs-target", "#pcollapse" + ProjectCounter);
    $("#accordionProj .accordion-button").last().attr("aria-controls", "pcollapse" + ProjectCounter);
  }
});
...
function updateCert() {
  for (let i = 0; i < $('#accordionCert .accordion-item').length; i++) {

    let a = ($(`#accordionCert .accordion-item:nth-child(${i + 1}) .Cert`).val().trim() == '') ? 'Cert' : $(`#accordionCert .accordion-item:nth-child(${i + 1}) .Cert`).val().trim();

    $(`#accordionCert .accordion-item:nth-child(${i + 1}) .accordion-button`).html(a);
  }
}

function cmakeVisible() {
  $("#accordionCert .accordion-header").css("display", "block");
  updateCert();
}

function delCert2(event) {
  event.preventDefault();
  if ($("#accordionCert .accordion-item").length > 1) {
    cmakeVisible();
    event.target.parentElement.parentElement.parentElement.remove();
  }
  event.stopPropagation();
...
let CertAdder = $("#accordionCert").html();
let CertCounter = 1;

$("#add_Cert").click(function (c) {
  let isValid = true;
  let finalValid = true;
  $("#accordionCert .accordion-item:last-child").find("input, select").each(function () { isValid = validate_chg_color(this); if (!isValid) { finalValid = false; } });
  if (!finalValid) {
    c.preventDefault();
  }
  else {
    updateCert();
    CertCounter++;
    if ($("#accordionCert .accordion-item").length > 0) {
      $("#accordionCert .accordion-header").css("display", "block");
      let count = $("#accordionCert .accordion-item").length;
      if (document.getElementById("accordionCert").getElementsByClassName("accordion-item")[count - 1].getElementsByClassName("accordion-collapse")[0].classList.contains("show")) {
        document.getElementById("accordionCert").getElementsByClassName("accordion-item")[count - 1].getElementsByClassName("accordion-button")[0].click();
      }
    }
    $("#accordionCert").append(CertAdder);
    $("#accordionCert .accordion-header").last().attr("id", "cheading" + CertCounter);
    $("#accordionCert .accordion-collapse").last().attr("aria-labelledby", "cheading" + CertCounter);
    $("#accordionCert .accordion-collapse").last().attr("id", "ccollapse" + CertCounter);
    $("#accordionCert .accordion-button").last().attr("data-bs-target", "#ccollapse" + CertCounter);
    $("#accordionCert .accordion-button").last().attr("aria-controls", "ccollapse" + CertCounter);
  }
});

$(".fc12").mouseleave(function () {
  if (Cert == 0) { return; }
  let timer = window.setTimeout(function () {
    cmakeVisible();
    let count = $("#accordionCert .accordion-item").length;
    for (let i = 0; i < count; i++) {
      if (document.getElementById("accordionCert").getElementsByClassName("accordion-item")[i].getElementsByClassName("accordion-collapse")[0].classList.contains("show")) {
        document.getElementById("accordionCert").getElementsByClassName("accordion-button")[i].click();
      }
    }
  }, 5000);
  $(".fc12").mouseenter(function () {
    window.clearTimeout(timer);
    $(".fc12").unbind('mouseenter');
  });
});
