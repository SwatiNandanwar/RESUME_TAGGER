...
function generateCV(template) {
    ...
    for (let i = 0; i < work_items; i++) {
     ...
     let Project_items = $('#accordionProj .accordion-item').length;

     for (let i = 0; i < Project_items; i++) {
         let Proj_title = $(`#accordionProj .accordion-item:nth-child(${i + 1}) .Proj_title`).val().trim();
         let Tech = $(`#accordionProj .accordion-item:nth-child(${i + 1}) .Tech`).val().trim();
         
         let proj_desc = $(`#accordionProj .accordion-item:nth-child(${i + 1}) .proj_desc`).val().trim();
         ...
         if (template == "Template_1") {
             $('.t1 .right_side .Proj').append(
                 `<div class="box">
                 <div class="year_company1">
             ...
             $('.t2 .lower_right .Proj.content').append(`<div class="con">
             <div class="time"><h4>${Tech}</h4></div>
             <div class="box"><div class="text">${Proj_title}</div><div class="exp">${proj_desc}</div></div>
         </div>`)
         }
         else if (template == 'Template_3') {
             $('.t3 .content-box .Proj').append(`<p class="Proj-title">${Proj_title}&nbsp;at&nbsp;${Tech}</p>
             <p class="par-4">${proj_desc}</p>`)
         }
     }
    ...
    let Cert_items = $('#accordionCert .accordion-item').length;
    for (let i = 0; i < Cert_items; i++) {
        let Cert = $(`#accordionCert .accordion-item:nth-child(${i + 1}) .Cert`).val().trim();

        if (Cert == "") {
            continue;
        }
        if (template == "Template_1") {
            $('.t1 .right_side .Cert .box').append(`<h4>${Cert}</h4>`);
        }
        else if (template == 'Template_2') {
            $('.t2 .lower .lower_left.Cert .content').append(`<div class="Cert">${Cert}</div>`)
        }
        else if (template == 'Template_3') {
            $('.t3 .Cert').append(`<li><span>${Cert}</span></li>`)
        }
    }
